...
EmberDreamFactoryAdapter.Serializer = DS.RESTSerializer.extend({
	...
	normalizeResponse: function(store, modelClass, payload, id, requestType) {
		var debug = true;
		console.group("normalizeResponse");
		console.log("store, modelClass, payload, id, requestType", store, modelClass, payload, id, requestType)

		var namespacedPayload = {};
		if( id ) {
			if( payload.record ) {
				namespacedPayload[modelClass.modelName] = payload.record[0];
			} else {
				namespacedPayload[modelClass.modelName] = payload;
			}
		} else {
			namespacedPayload[Ember.String.pluralize(modelClass.modelName)] = payload.record;
		}

		if( debug ) { console.log("namespacedPayload", namespacedPayload); }
		var response = this._super(store, modelClass, namespacedPayload, id, requestType);
...
EmberDreamFactoryAdapter.Adapter = DS.RESTAdapter.extend({
	...
	createRecord: function(store, type, record) {
		...
		return new Ember.RSVP.Promise(function(resolve, reject) {
			adapter.ajax(adapter.buildURL(type.modelName, id), "POST", { data: data }).then(function(json){
	...
	updateRecord: function(store, type, record) {
		...
		return new Ember.RSVP.Promise(function(resolve, reject) {
			...
			adapter.ajax(adapter.buildURL(type.modelName) + '?fields=*', "PUT", { data: data }).then(function(json){
				// if the request is a success we'll return the same data we passed in
				resolve(json);
			}, function(reason){
				reject(reason.responseJSON);
	...
	sessionToken: Ember.computed('headers.X-DreamFactory-Session-Token', {
		get(key) {
			return this.get('headers.X-DreamFactory-Session-Token');
		},
		set(key, value) {
			this.set('headers.X-DreamFactory-Session-Token', value);
			return value;
		}
	})
});
